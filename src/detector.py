...
class DeepfakeDetector:
    """Pretrained SBI deepfake detector built on EfficientNet-B4 via timm."""
    ...
    def __init__(self, weights_path: str):
        ...
        # strict=True now — keys should match exactly after stripping the `net.` prefix.
        missing, unexpected = self.model.load_state_dict(state_dict, strict=False)
        if len(missing) > 0:
            print(f"[detector] Warning: {len(missing)} missing keys during load.")
            print(f"[detector] Example missing: {missing[:3]}")
        if len(unexpected) > 0:
            print(f"[detector] Warning: {len(unexpected)} unexpected keys during load.")
            print(f"[detector] Example unexpected: {unexpected[:3]}")
        ...
        print(f"[detector] Backbone: efficientnet-b4 (efficientnet_pytorch, num_classes=2)")
        print(f"[detector] Parameters: {n_params/1e6:.2f}M")
    ...
    def predict_dataset(self, manifest_path: str) -> list[dict]:
        ...
        out_path = results_dir / f"{dataset_name}_predictions.json"
        with open(out_path, "w", encoding="utf-8") as f:
            # Strip the `frames` field from the saved file to keep it small.
            serializable = [
                {k: v for k, v in p.items() if k != "frames"} for p in predictions
            ]
            json.dump(serializable, f, indent=2)
        print(f"[detector] Wrote predictions -> {out_path}")
